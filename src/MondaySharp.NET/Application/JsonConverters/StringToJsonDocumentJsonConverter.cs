...
{
    internal class StringToJsonDocumentJsonConverter : JsonConverter<System.Text.Json.JsonDocument?>
    {
        public override JsonDocument? ReadJson(JsonReader reader, Type objectType, JsonDocument? existingValue, bool hasExistingValue, Newtonsoft.Json.JsonSerializer serializer)
        {
            ...
            if (reader.TokenType == JsonToken.Null || reader.TokenType == JsonToken.String && string.IsNullOrEmpty(reader.Value?.ToString())) return default;
